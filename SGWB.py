...
class SGWB(object):
    ...
    def __init__(self, fmax, fmin, fn, tsegmid, Ttot, omega0, alpha, nside_in,blm_vals,blmax,fref = 0.01, H0 = 2.2*10**(-18),
        ...
        frange = np.linspace(self.fmin,self.fmax,self.fn)
        tf = np.arange(0, Ttot, tsegmid)
        Omegaf = self.omega0 * (frange/self.fref)**(self.alpha)
        ...
        #det = TianQinOrbit(tf)
        self.TQresponse_plus = np.zeros((3,frange.size,tf.size,npix),dtype="complex")
        self.TQresponse_cross = np.zeros((3,frange.size,tf.size,npix),dtype="complex")
        for i in range(npix):
            for j in range(frange.size):
                TQreplus = response.trans_y_slr_fd(self.vec_k[:,i], self.e_plus[:,:,i], det, frange[j])[0]
                TQxyz_plus = response.get_AET_fd(TQreplus,frange[j],det.L_T)
                self.TQresponse_plus[:,j,:,i]  = TQxyz_plus 
                TQrecross = response.trans_y_slr_fd(self.vec_k[:,i], self.e_cross[:,:,i], det, frange[j])[0]
                TQxyz_cross = response.get_AET_fd(TQrecross,frange[j],det.L_T)
                self.TQresponse_cross[:,j,:,i] = TQxyz_cross
        self.TQ_ORF = (1/(8*np.pi))*(np.einsum("mjkl,njkl->mnjkl",np.conj(self.TQresponse_plus),self.TQresponse_plus)+np.einsum("mjkl,njkl->mnjkl",np.conj(self.TQresponse_cross),self.TQresponse_cross))
